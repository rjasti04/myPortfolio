...
**These docs are also too big to read whole.** Together they are ~117,500
...
| `docs/SECURITY.md` | ~7,500 | touching auth, session tokens, rate limits, the CSP, or any user-controlled output - it ends with a pre-merge checklist | `grep -n '^## '` (18 headings) |
| `docs/OPERATIONS.md` | ~5,500 | changing CI/CD, diagnosing a deploy, or running a manual procedure | `grep -n '^#\{2,3\} '` (28 headings) |
| `docs/TESTING.md` | ~15,000 | writing tests, or checking whether something is actually covered | `grep -n '^#\{2,3\} '` (14 headings) |
