...
**These docs are also too big to read whole.** Together they are ~112,000
...
That turns a 24,000-token read into roughly 1,000. Use it by default; read a
...
| `docs/JAVASCRIPT.md` | ~24,000 | adding or refactoring a frontend ES module | `grep -n '^### ' docs/JAVASCRIPT.md` (84 module sections) |
| `docs/FRONTEND.md` | ~15,500 | touching `index.html`, the CSS, the service worker, the fonts, or the build | `grep -n '^#\{2,3\} '` (22 headings) |
...
| `docs/TESTING.md` | ~14,000 | writing tests, or checking whether something is actually covered | `grep -n '^#\{2,3\} '` (14 headings) |
