...
**These docs are also too big to read whole.** Together they are ~114,500
...
That turns a 24,500-token read into roughly 1,000. Use it by default; read a
...
| `docs/API.md` | ~10,000 | adding or modifying a FastAPI route, or calling one from the client | `grep -n '^### ' docs/API.md` (39 endpoint sections) |
...
| `docs/SECURITY.md` | ~7,000 | touching auth, session tokens, rate limits, the CSP, or any user-controlled output - it ends with a pre-merge checklist | `grep -n '^## '` (18 headings) |
| `docs/OPERATIONS.md` | ~5,000 | changing CI/CD, diagnosing a deploy, or running a manual procedure | `grep -n '^#\{2,3\} '` (28 headings) |
| `docs/TESTING.md` | ~14,500 | writing tests, or checking whether something is actually covered | `grep -n '^#\{2,3\} '` (14 headings) |
