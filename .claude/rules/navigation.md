...
| :--- | ---: | ---: | :--- |
| `frontend/styles.css` | 13,568 | ~109,500 | `grep -n '#region' frontend/styles.css` returns a 27-entry map with live line numbers (~500 tokens). Then `sed -n 'START,ENDp'`. |
| `package-lock.json` | 3,453 | ~32,500 | Never read. `package.json` lists every direct dep in 25 lines. |
| `frontend/index.html` | 2,691 | ~40,500 | `grep -n '<section id=' frontend/index.html` for the 8-section map. |
...
| `frontend/three-bg.js` | 1,558 | ~14,500 | Animated plexus background. Despite the name it is plain 2D canvas — there is no Three.js in this repo. Read `docs/ARCHITECTURE.md` first to decide if you need it at all. |
