...
| :--- | ---: | ---: | :--- |
| `frontend/styles.css` | 13,656 | ~113,000 | `grep -n '#region' frontend/styles.css` returns a 28-entry map with live line numbers (~500 tokens). Then `sed -n 'START,ENDp'`. |
| `package-lock.json` | 3,453 | ~32,500 | Never read. `package.json` lists every direct dep in 25 lines. |
| `frontend/index.html` | 2,597 | ~39,500 | `grep -n '<section id=' frontend/index.html` for the 8-section map. |
