...
export function initThemeCustomizer() {
  ...
  const customizerDropdown = document.getElementById(
  ...
  const customizerContent = customizerDropdown.querySelector(
    ".customizer-content",
  );
  ...
  const colorPopoverSwatches = document.getElementById(
  ...
  function updateSelectedSwatches(value) {
    ...
    colorPopoverSwatches
      ?.querySelectorAll(".color-swatch-option")
      .forEach((button) => {
        button.classList.toggle(
          "is-selected",
          normalizeHex(button.dataset.color) === normalized,
        );
      });
  }
  ...
  function setColorValue(
    key,
    value,
    { preview = false, syncPopover = true } = {},
  ) {
    const normalized = normalizeHex(value);
    ...
    if (saved) {
      try {
        ...
        if (data.raw) {
          ...
          setColorValue(
            "secondary",
            data.raw.secondary || defaultColors.secondary,
          );
  ...
  colorPopoverHex?.addEventListener("blur", () => {
    if (
      !activeColorKey ||
      colorPopoverHex.getAttribute("aria-invalid") !== "true"
    )
      return;
  ...
  customizerDropdown.addEventListener("click", (event) => {
    ...
    if (
      target.closest(".color-input-wrapper") ||
  ...
  presetButtons.forEach((btn) => {
    btn.addEventListener("click", (e) => {
      ...
      if (colors) {
        setColorValue("primary", colors.primary, {
          preview: false,
          syncPopover: false,
        });
        setColorValue("secondary", colors.secondary, {
          preview: false,
          syncPopover: false,
        });
        setColorValue("accent", colors.accent, {
          preview: true,
          syncPopover: false,
        });
        closeColorPopover();
