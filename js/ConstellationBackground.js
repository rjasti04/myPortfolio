...
export class ConstellationBackground {
  ...
  updateNodeParallax() {
    ...
    for (const node of this.nodes) {
      const offsetX = this.mouse.x * parallaxStrength * (1 + node.position.z / 60);
      const offsetY = this.mouse.y * parallaxStrength * (1 + node.position.z / 60);
      ...
      if (Math.abs(node.userData.originalPos.x) > 50) node.userData.velocity.x *= -1;
      if (Math.abs(node.userData.originalPos.y) > 50) node.userData.velocity.y *= -1;
    }
  }
