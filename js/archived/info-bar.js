...
function getWeatherCode(code) {
  ...
  if (code === 1 || code === 2 || code === 3)
    return { icon: "fas fa-cloud-sun", desc: "Partly Cloudy" };
  if (code >= 45 && code <= 48) return { icon: "fas fa-smog", desc: "Fog" };
  if (code >= 51 && code <= 67)
    return { icon: "fas fa-cloud-rain", desc: "Rain" };
  if (code >= 71 && code <= 77)
    return { icon: "fas fa-snowflake", desc: "Snow" };
  if (code >= 80 && code <= 82)
    return { icon: "fas fa-cloud-showers-heavy", desc: "Showers" };
  if (code >= 95 && code <= 99)
    return { icon: "fas fa-bolt", desc: "Thunderstorm" };
