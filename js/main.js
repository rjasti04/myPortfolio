...
function loadActivityModule() {
  ...
  activityLoaded = true;
  activityModulePromise = import("./activity.js")
    .then((module) => {
      module.initActivity();
      return module;
    })
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (aiSection) {
    new MutationObserver(ensureChatForActiveAiSection).observe(aiSection, {
      attributes: true,
  ...
  if (activitySection) {
    new MutationObserver(ensureActivityForActiveSection).observe(
      activitySection,
      { attributes: true, attributeFilter: ["class"] },
    );
    window.addEventListener("hashchange", ensureActivityForActiveSection, {
      passive: true,
    });
...
function showUpdateNotification() {
  ...
  document
    .getElementById("update-refresh-btn")
