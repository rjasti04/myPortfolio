...
export function initParticles(containerId = "particles-canvas") {
  ...
  function connectParticles() {
    ...
    // Optimization: Pre-sort particles by X coordinate.
    // This allows us to break out of the inner loop early when the horizontal distance
    // exceeds our threshold, avoiding a full O(n^2) evaluation.
    particles.sort((a, b) => a.x - b.x);

    for (let i = 0; i < particles.length; i++) {
      for (let j = i + 1; j < particles.length; j++) {
        const dx = particles[j].x - particles[i].x;
        // Early break: since particles are sorted by X, all subsequent particles
