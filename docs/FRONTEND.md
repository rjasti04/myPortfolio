...
| Path | Notes |
| :--- | :--- |
| `profile-cutout-380.{png,webp}`, `profile-cutout-570.{png,webp}`, `profile-cutout.{png,webp}` | The landing portrait's **front** face — the outdoor shot, background-removed so the page's own gradient and backdrop show through the silhouette. Three widths × two formats, from `scripts/generate_profile_cutout.py`, which crops the bust out of a 3024×4032 master; 900 is the 2x of the 450px cap in `styles.css`, so the ladder now covers every width the `sizes` attribute can ask for. PNG rather than JPEG because the fallback has to carry an alpha channel. `profile-cutout-380.webp` is the LCP image **below 901px**: `index.html` preloads it under `media="(width <= 900px)"` and the build precaches it, and `scripts/tests/build.test.js` asserts those two stay the same file. Above that width this face is the one behind the turn |
...
| `profile-pic.jpeg` | The un-cut headshot at 725px, from `scripts/generate_profile_pics.py`, referenced by the JSON-LD `Person` image. The 160w and 360w variants and the WebP of each were published on every deploy and read by nothing once the landing view moved to the cutout above, so they and the ladder that produced them were removed |
...
`brush-backdrop.webp` is the block of paint behind the landing portrait, and
it paints **below 901px only** — the stacked layout, where it is the landing
view's only ground. Above that width the pseudo-element is never generated and
the hero wears a glass frame instead; see
[The hero glass frame](#the-hero-glass-frame). Nothing about the panel itself
differs between the two, because nothing about it needed to: every number in
it was measured against this layout. It is
...
`/arcade.html`, `/worldcup.html`, `/cron.html`, `/crypto.html`, `/json.html`
and `/diff.html` while developing. The links point at
`/ucl`, `/arcade`, `/worldcup`, `/cron`, `/crypto`, `/json` and `/diff` and will 404 on
