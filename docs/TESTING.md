...
| `crypto-hasher.test.js` | 56 | Real-time native `crypto.subtle` cryptographic digests (SHA-256, SHA-512, SHA-1) against NIST test vectors, uppercase hex formatting, and UTF-8 byte metric calculations |
| `crypto-generators.test.js` | 72 | RFC 4122 UUID v4 formatting and version/variant bits, RFC 9562 UUID v7 chronological sort ordering and 48-bit timestamp recovery, secure random tokens, and strong password generation |
