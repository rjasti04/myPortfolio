...
| `pull-to-refresh.test.js` | 159 | The gesture that replaces the browser's own on the AI panel, which is all guards: a drag past the threshold reloads and a shorter one settles back; a gesture starting below the top of the scroller is ignored, so a flick back to the top is not a refresh; an upward drag is handed back uncancelled; a second finger drops the pull; the pull follows only the finger it started with; travel is capped; and `destroy` takes the listeners and the indicator with it |
