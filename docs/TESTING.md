...
| `chat.js` and `auth-ui.js` have no direct unit tests | Both are ~1,200-line single initialisers | `auth-refresh.test.js` and `auth-register.test.js` cover the riskiest shared paths in `auth.js`; the backend contract test covers the endpoints they call. The register panel's success painting is verified by reading only |
