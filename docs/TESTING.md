...
| `apps-experience.test.js` | 401 | **The Apps filter and the Experience groups.** Every app tile carries a `data-app-category` and every category has a chip, filtering shows only that category with exactly one chip pressed, re-pressing the active chip clears it, each change is announced with correct singular/plural, and the chip counts match the tiles present. For Experience: the groups are `<details>` with the first open, the palette anchor sits on the `<details>` rather than the heading so it can be opened, every `exp-` anchor in `search-index.js` resolves to one, expand-all reports the next action in its label and the set state in `aria-expanded`, focusing a closed group opens it (the palette deep-link path), and every section lede reaches the Ctrl+K index Also the shelf's images and per-tile signals: every tile serves WebP through `<picture>` at two widths with `sizes`, every referenced file ships and the seven thumbnails together come in under 600 KB (the PNGs they replaced were ~1.93 MB), the OG cards stay PNG because a social card is a different job, the scrim that held their baked-in copy back is gone with them, the filter is addressable (`#apps/dev-tools` opens filtered, a chip press rewrites the fragment, an unknown slug falls back to All rather than emptying the grid, and the router resolves a section from the first segment only), and every tile carries two or three non-repeating trait chips above its Launch pill - with "Nothing stored" only claimed by an app that has an opt-in to be the exception |
...
| `activity-lifecycle.test.js` | 161 | **Activity lifecycle regression (C13).** Drives `initActivity()` through the `active` class the router toggles, with a fake `EventSource`: leaving while the first load is in flight opens no stream and leaves no timer; a source the browser has `CLOSED` reports "error" at once; Refresh reopens a dead stream and leaves a healthy one alone |
| `syntax-highlighter.test.js` | 40 | **`$`-pattern regression (C16).** Source containing `$&`, `` $` ``, `$'`, `$$` and a template literal's `$${…}` renders exactly as written, and the tokens around it are still highlighted |
...
| `owner-analytics.test.js` | 197 | The owner panel asks for nothing when signed out; a 403 leaves the section as a visitor sees it with no empty shell; all six cards render for the owner; mistyped terminal commands are flagged; a failed panel is a `role="alert"` card rather than zeroes; the window picker re-requests every panel; signing out takes the panel down; only the latest window click paints - a 90-day answer landing after a 7-day one is discarded and the busy state waits for the right answer (C14) |
...
| `chat-history-sync.test.js` | 472 | Server-side conversation history in the chat UI: every session gets a v4 UUID the route will parse; pre-`conversationId` sessions are backfilled without moving their local key; no history call when signed out; server conversations merge into the rail; a stub fetches its transcript only when opened; a failed load renders an error with a retry rather than an empty conversation; a 404 stub is dropped; delete removes the server copy; an unreachable API leaves local conversations intact; stored history that parses but is not an array (null, an object, a number, a string) is replaced rather than taking `initChat` down, and non-object entries are dropped. **C5/C11:** the per-view `hydrating`/`loadError` flags are never persisted; a row an older build stored mid-hydration still fetches; an active stub is fetched on load rather than left spinning, and says "Sign in" when it cannot be; a 404 on open repaints the newly active conversation; a failed server delete (500, 429) shows the may-reappear notice and a 404 does not; at the 50-row cap server stubs are evicted before local-only conversations |
| `chat-stop.test.js` | 219 | **Stop and busy-state regression (C12).** Drives the AI page's composer: two same-tick submits on a first visit make one `/chat/stream` request; Stop during a summary aborts it and nothing streams; Stop before the headers, and before the first token, end the turn with no error card and no Retry |
...
| `analytics-session-headers.test.js` | 47 | `sessionHeaders()` returns the session id and its token, and `{}` before a session exists - what lets a chat turn be attributed to a session at all |
| `analytics-unload-flush.test.js` | 169 | **Hide/unload flush regression (C10).** A fresh window per test: a hide flush that cannot be sent keeps the queue in memory and persisted; one that succeeds removes only what it sent and keeps events queued during the flight; a second hide does not resend what is in flight; an unload of a ~150 KiB backlog sends a body within the 60 KiB budget, oldest first, and leaves the rest persisted |
| `sw-update.test.js` | 115 | **Update-prompt regression (C15).** Imports the real `main.js`: a worker already waiting at load is offered and Refresh posts `SKIP_WAITING`; no waiting worker offers nothing; a rejected lazy import shows one banner however often it fails, while a throwing initialiser and an offline failure show none |
| `auth-refresh.test.js` | 401 | **BUG-03 regression.** Concurrent 401s share one refresh instead of racing rotation; a later 401 starts a fresh refresh; a genuinely failed refresh signs the user out exactly once; a 429, 5xx or dropped connection during refresh keeps the session, while a 403 still ends it. **Cross-tab (C1):** two module instances over one storage share one refresh under a fake lock manager; without one, losing the race adopts the winner's pair; a request that failed before another tab rotated retries without refreshing. `isRejectedResponse` separates a transient 401 from a refused credential (C4); a 400 from a re-authenticated route is not refreshed or retried (C2); logout sends the refresh token in the body with no bearer, falls back to the bearer only without one, and signs the device out even offline (C3) |
| `auth-nav-session.test.js` | 158 | **Header session regression (C4, C14(b)).** Drives the real `index.html`: when `/auth/me` 401s and the refresh behind it answers 429 or 5xx, `setupNavUI` keeps both tokens, and a refused refresh token still clears them; however often the header re-renders, only the latest render's outside-click listener is live |
| `auth-2fa.test.js` | 289 | **Half-built-surface regression.** `disable2FA` was defined in `js/auth.js`, exported, and imported by nobody, while the nav item's label switched to "2FA Enabled" and its handler still dispatched the enrolment event - so an enrolled user got the enrolment panel, the server's refusal to reissue a live secret, an empty `<img>` and a bare "Secret Key:", plus an error naming a Disable control that did not exist. Drives the real `index.html` under jsdom: an unenrolled account reaches `/2fa/setup` and renders the QR and secret, an enrolled one routes to the manage panel and never calls setup, leaving the panel drops the secret out of the DOM (and leaves no empty `src`, which refetches `index.html` as an image), the disable form posts `{current_password, code}`, the disable button stays operable and explains what is missing, and a failed login challenge clears the spent pre-auth token and returns to the login tab |
...
| `ucl-bracket.test.js` | 756 | The Champions League predictor end to end in jsdom: the 36-row table renders in its three qualification zones with each club's association, the play-off ties pair seeds 9–16 against 17–24, the round of 16 seeds the top eight against the reserved bands, a chalk bracket crowns the top seed, the `?s=` share code round-trips, reordering the table clears picks that no longer exist, a malformed share code falls back to the default table, quick-fill settles the remaining ties without overwriting existing picks, and the rank pill's jump field clamps out-of-range positions and cancels on Escape. Plus the accessibility pass it had never had: the stage switcher is a real tablist with one `aria-selected` tab and one Tab stop, clicking and arrowing both move selection and focusability together, the set wraps, the mobile round switcher is a tablist whose single panel's label follows the selected tab, every stage panel is a named `tabpanel`, no decorative `<i class="fa…">` is left exposed, Reset takes two presses instead of raising `window.confirm` (which jsdom answers `false`, so it could never be exercised at all), and the page loads the shared chrome |
| `worldcup-bracket.test.js` | 311 | The 2026 World Cup predictor end to end in jsdom: 12 groups of 4 teams, 12 wildcard candidates (8 advancing), all 32 knockout ties rendered, random fill completes undecided matches and crowns champion and podium, manual picks preserved, URL state parameter round-trips, and consecutive runs produce varied outcomes. Plus the accessibility pass and the parity with `/ucl` it was a generation behind on: a real tablist for both the stage and mobile round switchers, no exposed decorative icon, the header no longer branding the app with the mark its footer disclaims, three hero context chips, a progress bar over the whole tournament (eight wildcard places plus 32 knockout ties) that reads 8 of 40 at rest and complete after a fill, reorder buttons that name the club they move rather than saying "Move team up" twelve groups over, and a two-press Reset |
...
| `theme-randomizer.test.js` | 167 | `randomPalette`, the theme customiser's **Randomize** button. Testing a random function is worth it precisely because it is constrained: over 400 rolls per case, every colour is a six-digit hex, saturation and lightness stay inside the bands `generateVariants` can derive usable light and dark variants from, the new primary always lands at least 35 degrees from the palette it replaced, and every hue gap sits within a degree of one of the five offsets the fixed harmonies allow, with all five reached, rather than spreading like three independent draws. That last case used to count distinct gaps rounded to whole degrees and allow up to 12; the eight-bit hex round-trip moves a recovered hue by up to ~0.76 degrees, which makes the true set 14 rather than 5 and made the assertion fail on roughly 1.5% of runs. Snapping each gap to its nearest offset tests the same property deterministically, and more strictly |
...
| `app-shared.test.js` | 233 | **The header app switcher and the `?` shortcuts sheet, shared by all seven apps.** The switcher's app list is asserted against `index.html`, so it and the shelf cannot drift; exactly one entry is `aria-current`; every app it lists is a page that ships; the Back link survives as a link; the menu closes on Escape and returns focus to its trigger. For the sheet: its row count equals the registered bindings plus the two this module owns, so it can only list what is actually bound; `?` toggles and Escape closes; both stay out of the way inside a text field or under a modifier; `/` focuses the primary input and a digit switches tab; `suppress` hands the keyboard back entirely (the arcade, mid-game) and closes an open sheet; a documentation-only entry is listed and binds nothing. Finally every app links `app-shared.css`, and that sheet names only the seven-token chrome contract all seven of them define |
| `app-about.test.js` | 188 | **The About tab on /cron, /crypto, /json and /diff, and /diff's regrouped options.** Each About tab is wired to a real `tabpanel`, is not the tab the app opens on, carries a Limits section and states `connect-src 'none'`, and each entry point routes to it; the `.prose-*` treatment lives in `app-chrome.css` rather than four copies. Also: /json's Compare tab and collapsible source card are wired and the status strip stays outside the collapsing part; /diff's options are three named groups with the four "ignores" under Compare and "Remember my panes" under Session; /diff has swap and a sample whose two sides actually differ; `j`/`k` are registered through the shortcuts table rather than bound twice; and the brand titles stand alone at the width where `.brand-tag` is hidden |
...
| `cron.test.js` | 221 | 5-part POSIX cron syntax validation, step and range parsing, month and day-of-week name normalization, plain-English translation generation, and sequential next-trigger timeline calculation Plus macros and timezones: each of the six standard macros expands to its canonical five fields and parses identically to the expression it stands for, case-insensitively and through the trigger timeline too; an unknown macro is named and `@reboot` says it has no schedule rather than failing on field count; a 6- or 7-field expression is named as Quartz while any other wrong count gets the plain message; and the timeline is genuinely zone-aware - the same expression fires at different instants in UTC and New York, the wall-clock hour is whatever the visitor asked for, and a daylight-saving jump neither drops nor duplicates a daily run |
...
| `crypto-hasher.test.js` | 124 | Real-time native `crypto.subtle` cryptographic digests (SHA-256, SHA-512, SHA-1) against NIST test vectors, uppercase hex formatting, and UTF-8 byte metric calculations, plus HMAC against RFC 4231 (SHA-256, SHA-512) and RFC 2202 (SHA-1) vectors, a different key giving a different digest, hex/uppercase/base64 all being one value, an unsupported algorithm refused rather than silently downgraded, and every offered algorithm actually working |
| `crypto-jwt.test.js` | 101 | **The JWT decoder on /crypto.** RFC 7519's own example token decodes; `exp` is read as NumericDate seconds rather than milliseconds (a token read as 1970 is the classic symptom of missing that); expiry is answered in both directions and left `null` when there is no `exp`; registered claims are annotated and unknown ones left alone; an object claim is stringified rather than rendered as `[object Object]`; non-ASCII claims survive the Base64URL round trip; six kinds of malformed token are reported rather than thrown, with the field count named because a truncated paste is the usual cause; and the panel states it does not verify a signature and offers no control that would appear to |
| `crypto-share.test.js` | 98 | **The share link on /crypto.** The URL carries the tab and the settings, and carries **nothing that was typed** - not the plaintext, not an HMAC key, not a token, not an encoder payload, which is the property that makes the button safe to paste into a ticket; an unknown tab falls back rather than producing a dead link; stale query parameters are dropped rather than accumulated; a shared link restores the controls it names; a value the page does not offer is dropped rather than blanking a `<select>`; a range outside its own bounds is refused; the button no longer says "permalink"; and Clear now sits in `header-actions` with the privacy sentence still in the footer |
...
| `json-compare.test.js` | 97 | **Structural JSON comparison on /json.** Key order is not a difference (the whole reason the tab exists) but array order is; whitespace and indentation never are; turning the switch off does report key order; a real change is counted as a modification and an added key as an addition; a document that does not parse is reported per side rather than thrown; canonical text sorts keys at every depth, leaves arrays alone and keeps `null` as `null`; rows carry a unified-diff hunk header and both line numbers with the absent side `null`; identical documents produce no rows; and the row cap holds against a single change carrying hundreds of lines |
...
| `test_chat_stream.py` | 17 | Streaming success and metrics; Converse for a non-Anthropic model; consecutive user messages merged; invalid model rejected; **streaming does not block the event loop**; the anonymous free-message cap and its non-application to signed-in callers, on `/chat/stream` and on `/chat/summarize` (refused before a slot is taken); `system_prompt` override refused; oversized message, oversized conversation, too many messages and unknown role rejected; a normal conversation still fits; **a verified session id reaches Bedrock as `requestMetadata`** on Converse, InvokeModel and summarize, and a forged or absent one does not |
...
| `animations.js` is untested | Animation is not asserted | Manual review; it is self-contained and degrades to a no-op |
