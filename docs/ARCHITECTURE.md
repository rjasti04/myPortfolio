...
| `js/arcade/` | The `/arcade` page's own modules — a shell, a loop/canvas helper, input binders, storage, a Web Audio synth and five games. Built as a separate esbuild entry point; shares nothing with the SPA |
...
| `arcade.html` + `arcade.css` | Standalone games page served at `/arcade` — 2048, Tetris, Flapper, Stack and Snake, each addressable by fragment (`/arcade#snake`). Reached from the SPA's **Apps** section, which links out to it in a new tab. Unlike the two predictors it keeps the SPA's no-third-party rule: it links the site's own self-hosted `fonts.css` and draws its icons as inline SVG, and its own CSP is `script-src 'self'` with no inline script to hash |
