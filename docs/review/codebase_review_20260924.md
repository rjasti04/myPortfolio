...
| **Status** | ✅ **Resolved** |
| **What changed** | Both send buttons set `aria-label` with their `title` ("Stop generating" / "Send message"), and their glyphs are `aria-hidden`. `aria-busy` moved from the inputs and forms to the two message lists while a reply streams; "Response received" is still announced once. A truncated reply now carries visible text, "Cut off at the length limit", with no token count: the server's ceiling is 2048, the old tooltip said 2000, and ADR-023 makes the server the owner of that number. `announceToScreenReader` writes to `#route-announcer`, clearing it first so a repeated message is still read. `chat-stop.test.js` covers the label, the busy list, the note and the announcement. Real screen-reader behaviour is still unverified, as the report says. |
...
| **Status** | ✅ **Resolved** |
| **What changed** | `/worldcup`'s state reads and writes go through guarded `readStored`/`writeStored`/`removeStored`, as `/ucl`'s did. `/crypto` and `/cron` read the saved theme with `json-main.js`'s guard, accepting only `"light"` or `"dark"`. Fixed in place, in each file's idiom - no shared helper, since the predictors cannot import. Tests boot each page with a `localStorage` that throws: `/worldcup` renders and takes a pick, `/crypto`'s tabs switch, `/cron`'s theme toggles. |
...
| **Status** | ✅ **Resolved** |
| **What changed** | (1) `/crypto` and `/json`'s file inputs are `aria-hidden` and `tabindex="-1"`. (2) Diff's Split/Unified buttons carry `aria-pressed`, set in the markup, on restore and on click. (3) The `?` sheet traps Tab locally - not with `modal.js`, which belongs to the SPA's build group. (4) The Apps switcher is a `<nav>` disclosure of links with no `menu`/`menuitem`/`aria-haspopup`; opening focuses the first link. (5) `/crypto` announces through one `#crypto-status` region, on transitions only: an error appearing or changing, a converted file, an explicit Generate. (6) Every two-press confirm - both predictors and the three Dev Tools apps - swaps its `aria-label` to "…: press again to confirm" while armed and restores it after. Tests in `app-shared.test.js` and both bracket files. |
...
| **Status** | ✅ **Resolved** |
| **What changed** | `triggerConfetti` returns early under reduced motion, and `/worldcup`'s podium celebration does too. The JS smooth scrolls in `chat.js`, `terminal/palette.js`, `cron-main.js`, `crypto-main.js` and `diff-ui.js` pass `"auto"` then - including two tab-strip `scrollIntoView` calls (`cron-main.js`, `crypto-main.js`) the report did not list. `/worldcup` gets `/ucl`'s reduced-motion block. **Differs from the suggested fix:** `config.js` exports `prefersReducedMotion` as a `MediaQueryList`, so the SPA reads `.matches`; the Dev Tools apps and `palette.js` read `matchMedia` at call time instead, because the apps' build group does not import SPA modules and `palette-search.test.js` loads `palette.js` before any `window` exists. The block shortens durations rather than setting `animation: none`, which would never fire the `animationend` that removes `/worldcup`'s toasts. |
...
| **Status** | ✅ **Resolved** |
| **What changed** | Copy fallbacks in `cron-ui.js`, `regex-ui.js` and `crypto-ui.js` use `execCommand`'s return value and say "Copy failed"; the predictors put the link in the address bar with a warning toast instead of `alert()`/`prompt()`. The chat header reads Online, Offline or Unavailable, green only when online. `/cron`, `/ucl` and `/worldcup` toasts gain a close button, hold on hover or focus, and last 3.2 s. `/diff` reports a refused or unreadable file in a `role="status"` note under the pane, not `alert()`. `/` returns before `preventDefault()` when its input sits in a hidden panel. **Differs from the suggested fix:** the SPA's toast cannot be imported by the predictors (single files) or the Dev Tools apps (a separate build group), so its behaviour is ported into each app's own toast. |
...
| **Status** | ✅ **Resolved** |
| **What changed** | Arcade's Back link is a `.back-btn`, so the switcher mounts there, and `arcade.css` styles the trigger as one of its own `.btn`s. **Differs from the suggested fix:** `.back-btn` alone would have mounted an unstyled browser button, because the trigger is an `.action-btn` and only `app-chrome.css` styles that. The four Dev Tools pages load `js/app-shared/theme-prepaint.js` in their `<head>` before any stylesheet. **Differs from the suggested fix:** the report said the snippet needs a CSP hash "only if those pages gain a CSP" - they already have one, with `script-src 'self'`, so it is a file built as an IIFE like `theme-bootstrap.js`, not an inline script. Every confirm now uses `.is-confirming` and 4 s, and `app-chrome.css`'s `.confirming` alias is gone. `build.test.js` checks each shipped page loads the hashed script ahead of its CSS. |
