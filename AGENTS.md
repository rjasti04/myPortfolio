...
| :--- | ---: | ---: | :--- |
| `frontend/styles.css` | 11,874 | ~87,500 | `grep -n '#region' frontend/styles.css` returns a 27-entry map with live line numbers (~500 tokens). Then `sed -n 'START,ENDp'`. |
...
**These docs are also too big to read whole.** Together they are ~77,000
...
That turns a 15,500-token read into roughly 500. Use it by default; read a
...
| `docs/JAVASCRIPT.md` | ~15,500 | adding or refactoring a frontend ES module | `grep -n '^### ' docs/JAVASCRIPT.md` (51 module sections) |
