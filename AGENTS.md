...
| :--- | ---: | ---: | :--- |
| `frontend/styles.css` | 10,221 | ~69,500 | `grep -n '#region' frontend/styles.css` returns a 26-entry map with live line numbers (~500 tokens). Then `sed -n 'START,ENDp'`. |
...
| `frontend/js/chat.js` | 1,593 | ~16,000 | One large `initChat()` from line 57; almost nothing is top-level. Map it with `grep -nE '^\s{2,6}(async )?function \w+' frontend/js/chat.js` (24 hits). |
