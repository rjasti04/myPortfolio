...
| :--- | ---: | ---: | :--- |
| `frontend/styles.css` | 12,113 | ~89,000 | `grep -n '#region' frontend/styles.css` returns a 27-entry map with live line numbers (~500 tokens). Then `sed -n 'START,ENDp'`. |
...
**These docs are also too big to read whole.** Together they are ~84,500
...
| `docs/FRONTEND.md` | ~11,500 | touching `index.html`, the CSS, the service worker, the fonts, or the build | `grep -n '^#\{2,3\} '` (20 headings) |
...
| `docs/TESTING.md` | ~7,500 | writing tests, or checking whether something is actually covered | `grep -n '^#\{2,3\} '` (13 headings) |
