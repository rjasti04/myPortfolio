...
| :--- | ---: | ---: | :--- |
| `frontend/styles.css` | 10,948 | ~77,500 | `grep -n '#region' frontend/styles.css` returns a 26-entry map with live line numbers (~500 tokens). Then `sed -n 'START,ENDp'`. |
...
**These docs are also too big to read whole.** Together they are ~64,000
...
That turns a 9,000-token read into roughly 500. Use it by default; read a
...
| `docs/JAVASCRIPT.md` | ~9,000 | adding or refactoring a frontend ES module | `grep -n '^### ' docs/JAVASCRIPT.md` (37 module sections) |
