...
| :--- | ---: | ---: | :--- |
| `frontend/styles.css` | 11,531 | ~84,500 | `grep -n '#region' frontend/styles.css` returns a 27-entry map with live line numbers (~500 tokens). Then `sed -n 'START,ENDp'`. |
| `package-lock.json` | 3,453 | ~32,500 | Never read. `package.json` lists every direct dep in 25 lines. |
| `frontend/index.html` | 2,328 | ~35,500 | `grep -n '<section id=' frontend/index.html` for the 8-section map. |
...
**These docs are also too big to read whole.** Together they are ~69,500
...
That turns a 10,500-token read into roughly 500. Use it by default; read a
...
| `docs/JAVASCRIPT.md` | ~10,500 | adding or refactoring a frontend ES module | `grep -n '^### ' docs/JAVASCRIPT.md` (38 module sections) |
| `docs/FRONTEND.md` | ~10,000 | touching `index.html`, the CSS, the service worker, the fonts, or the build | `grep -n '^#\{2,3\} '` (20 headings) |
...
| `docs/TESTING.md` | ~5,500 | writing tests, or checking whether something is actually covered | `grep -n '^#\{2,3\} '` (13 headings) |
...
| `README.md` | ~4,500 | human onboarding only; the stack summary above supersedes it | Small enough to read whole |
