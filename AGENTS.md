...
| :--- | ---: | ---: | :--- |
| `frontend/styles.css` | 10,993 | ~78,000 | `grep -n '#region' frontend/styles.css` returns a 27-entry map with live line numbers (~500 tokens). Then `sed -n 'START,ENDp'`. |
| `package-lock.json` | 3,453 | ~32,500 | Never read. `package.json` lists every direct dep in 25 lines. |
| `frontend/index.html` | 2,062 | ~32,000 | `grep -n '<section id=' frontend/index.html` for the 8-section map. |
...
| `frontend/three-bg.js` | 1,517 | ~14,000 | Animated plexus background. Despite the name it is plain 2D canvas — there is no Three.js in this repo. Read `docs/ARCHITECTURE.md` first to decide if you need it at all. |
...
**These docs are also too big to read whole.** Together they are ~64,500
