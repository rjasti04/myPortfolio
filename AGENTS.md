...
| :--- | ---: | ---: | :--- |
| `frontend/styles.css` | 11,418 | ~83,000 | `grep -n '#region' frontend/styles.css` returns a 27-entry map with live line numbers (~500 tokens). Then `sed -n 'START,ENDp'`. |
| `package-lock.json` | 3,453 | ~32,500 | Never read. `package.json` lists every direct dep in 25 lines. |
| `frontend/index.html` | 2,276 | ~35,000 | `grep -n '<section id=' frontend/index.html` for the 8-section map. |
...
**These docs are also too big to read whole.** Together they are ~68,000
...
| `docs/JAVASCRIPT.md` | ~10,500 | adding or refactoring a frontend ES module | `grep -n '^### ' docs/JAVASCRIPT.md` (37 module sections) |
